...
architecture rtl of normalize_to_fb is
    ...
    -- Counters
    ---------------------------------------------------------------------------
    signal pt_idx : unsigned(14 downto 0) := (others => '0');
    signal fb_idx : unsigned(17 downto 0) := (others => '0');

    ---------------------------------------------------------------------------
    -- One-cycle BRAM read buffer
    ---------------------------------------------------------------------------
    signal u_buf16, v_buf16 : signed(15 downto 0) := (others => '0');
    ...
    ---------------------------------------------------------------------------
    -- Range, scale, dynamic offsets
    ---------------------------------------------------------------------------
    signal range_u32, range_v32     : signed(31 downto 0);
    signal scale_w_q1616, scale_h_q1616 : signed(31 downto 0);
    signal scale_q1616              : signed(31 downto 0);
    signal off_x_int, off_y_int     : signed(15 downto 0);   -- dynamic!

    ---------------------------------------------------------------------------
    -- Per-point pipeline regs
    ---------------------------------------------------------------------------
    signal diff_x32, diff_y32        : signed(31 downto 0);
    signal norm_x_q1616, norm_y_q1616: signed(31 downto 0);
    ...
    constant FB_W_U18  : unsigned(17 downto 0) := to_unsigned(FB_W,18);
begin
    ---------------------------------------------------------------------------
    process(clk)
        variable addr36 : unsigned(35 downto 0);
        variable px, py : signed(15 downto 0);
            ...
            if rst = '1' then
                ...
                when DIV_W =>
                    if range_u32 = 0 then
                        ...
                        scale_w_q1616 <= shift_left(to_signed(FB_W-1,32),FRAC_BITS) / range_u32;
                    end if;
                    st <= DIV_H;
                ...
                when DIV_H =>
                    if range_v32 = 0 then
                        ...
                        scale_h_q1616 <= shift_left(to_signed(FB_H-1,32),FRAC_BITS) / range_v32;
                    end if;
                    st <= SELECTING;
                ...
                ----------------------------------------------------------------
                -- compute dynamic centre offsets
                ----------------------------------------------------------------
                when OFFSET_PREP =>
                ...
                when MUL =>
                    norm_x_q1616 <= resize(diff_x32 * scale_q1616,32);
                    norm_y_q1616 <= resize(diff_y32 * scale_q1616,32);
                    st <= CLAMP;

                when CLAMP =>
                    px := resize(norm_x_q1616(31 downto FRAC_BITS),16) + off_x_int;
                    py := resize(norm_y_q1616(31 downto FRAC_BITS),16) + off_y_int;
                    if px < 0 then px := to_signed(0,16); end if;
                    if px > to_signed(FB_W-1,16) then px := to_signed(FB_W-1,16); end if;
                    if py < 0 then py := to_signed(0,16); end if;
                    if py > to_signed(FB_H-1,16) then py := to_signed(FB_H-1,16); end if;
                    pix_x_reg <= px;
                    pix_y_reg <= py;
                    st <= WRITE_FB;

                when WRITE_FB =>
                    addr36 := unsigned(resize(pix_y_reg,18))*FB_W_U18 +
                              unsigned(resize(pix_x_reg,18));
                    fb_addr <= std_logic_vector(addr36(17 downto 0));
                    fb_we   <= '1';
                    if pt_idx = to_unsigned(DEPTH_3D-1, pt_idx'length) then
                        st <= FINISHED;
                    else
